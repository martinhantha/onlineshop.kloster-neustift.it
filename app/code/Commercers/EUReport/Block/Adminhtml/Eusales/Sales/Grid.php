...
namespace Commercers\EUReport\Block\Adminhtml\Eusales\Sales;
...
class Grid extends \Magento\Reports\Block\Adminhtml\Grid\AbstractGrid
{
    ...
    public function getResourceCollectionName()
    {
        return $this->getFilterData()->getData('report_type') === 'updated_at_order'
            ? \Commercers\EUReport\Model\ResourceModel\Report\Order\Updatedat\Collection::class
            : \Commercers\EUReport\Model\ResourceModel\Report\Order\Collection::class;
    }
    ...
    protected function _prepareColumns()
    {
        ...
        $this->addColumn(
            ...
            [
                'header' => __('Sales Total (excl. VAT)'),
                'type' => 'currency',
                'currency_code' => $currencyCode,
                'index' => 'total_amount_excl_tax',
                'total' => 'sum',
                'sortable' => false,
                'renderer' => Currency::class,
                'rate' => $rate,
                'header_css_class' => 'col-sales-total-excl-tax',
                'column_css_class' => 'col-sales-total-excl-tax'
            ]
        );
        ...
        $AllCountry = $this->_countryCollection->loadData()->toOptionArray();
        $options = [];
        foreach ($AllCountry as $country) {
            if (!empty($country['value'])) {
                $options[$country['value']] = __($country['label']);
            }
        }
        $this->addColumn(
            'shipping_country',
...
//            $this->getCollection()->setColumnGroupBy('shipping_country');
//        }
    ...
    protected function _addCustomFilter($collection, $filterData)
    {
        ...
        if ($filterData->hasData('eu_shipping_country')) {
            $collection->addFieldToFilter('shipping_country', ['in' => $filterData->getData('eu_shipping_country')[0]]);
        }

        $collection->addFieldToFilter('included_tax', $filterData->getData('is_included_tax'));
